 ...
 * 
 * 【更新履歴】
 * 1.22 2023/02/11 「無効:hide後に不透明度初期化」追加。
 * 1.21 2022/09/29 「立ち絵のピクチャ操作無効」追加。
 * 1.06 2021/4/9   「WebGLでのToneFilter無効」追加。
 ...
 *
 * @param disableWebGLTone
 * @text WebGLでのToneFilter無効
 * @desc ON/trueでWebGLモード時に立ち絵の色調変化に使用するToneFilterを無効化
 * @default false
 * @type boolean
 *
 * @param disablePictureMove
 * @text 立ち絵のピクチャ操作無効
 * @desc ピクチャ移動による立ち絵操作を無効化。一部ピクチャ系プラグインとの競合対策用（競合対象より後ろに配置）
 * @default false
 * @type boolean
 *
 ...
 * @parent categoryDisappearCommands
 *
 * @param disableClearOpacityAfterHide
 * @text 無効:hide後に不透明度初期化
 * @desc ONで非表示(hide)後に不透明度を初期化する機能を無効化
 * @default false
 * @type boolean
 * @parent categoryDisappearCommands
 *
