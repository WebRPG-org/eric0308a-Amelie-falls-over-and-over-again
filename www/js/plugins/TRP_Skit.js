 ...
 *
 * 【更新履歴】
 * 1.22 2023/02/11 hide後に不透明度を初期化するよう修正。(設定で無効化可)
 * 1.21 2022/09/29 設定「立ち絵のピクチャ操作無効」に対応(競合対策用。順番影響するので注意)
 * 1.20 2021/7/18  立ち絵が非表示後にアニメーションが残る不具合修正
 * 1.19 2021/4/11  一部ピクチャ関連プラグインとの競合対応
 * 1.18 2021/4/9   WebGL使用時に色調変更にToneFilterを使用
 * 1.17 2020/6/17  一部コマンドのall対象に対応
 * 1.16 2020/6/13  bitmapロード周りのコード修正
...
TRP_CORE.setupTRPSkitConfigParametersIfNeeded = function(){
	...
	parameters = TRP_CORE.skitParameters = JSON.parse(JSON.stringify(TRP_CORE.skitParameters, function(key, value) {
		try {
...
Skit.prototype.playLoop = function(args,macroPos){
	...
	var targetActors = this.targetActors(args[1]);
	var length = targetActors.length;
	var macroNames,position;
	var firstCommand = true;
    for(var i = 0; i<length; i=(i+1)|0){
        var actor = targetActors[i];
        if(actor){
        	if(CLEAR_LOOP_ARGS.contains(args[3])){
				wait = firstCommand && args[2];
				actor.stopLoop(wait,TRP_CORE.supplementDefBool(false,args[4]));
			}else{
				if(firstCommand){
					wait = false;
					position = TRP_CORE.supplementDef(args[3],macroPos);
					macroNames = args.slice(4);
				}else{
					macroNames = macroNames.concat();
				}
...
Skit.prototype.actorsForHideCommands = function(name){
	var actors;
	if(Skit.ALL_TARGET_NAMES.contains(name)){
...
Skit.prototype.targetActors = function(name){
	var ret;
	if(Skit.ALL_TARGET_NAMES.contains(name)){
		ret = this.showingActors();
	}else{
		ret = Skit.TEMP_TARGET_ARRAY;
		ret.length = 0;
		ret.push(this.actor(name));
	}
	return ret;
};
...
Sprite_Picture.prototype.initialize = function(pictureId){
	...
	this.initAnimation();
	if(parameters.disableWebGLTone==='true'||parameters.disableWebGLTone===true){
		this._useToneFilter = false;
	}else{
		this._useToneFilter = this._isBustPicture && Graphics.isWebGL();
	}
...
Sprite_Picture.prototype.updateOverlay = function(){
	...
	var tone = this._useToneFilter ? null : (picture.tone() || Sprite_Picture.DEFAULT_TONE);
...
Sprite_Picture.prototype._needsTint = function(){
	...
	return _Sprite_Picture_needsTint.call(this);
};

var _Sprite_Picture_setColorTone = Sprite_Picture.prototype.setColorTone;
Sprite_Picture.prototype.setColorTone = function(tone) {
	if(!this._useToneFilter){
		_Sprite_Picture_setColorTone.call(this,tone);
	}else{
	    ...
	    if (!this._colorTone.equals(tone)) {
	        ...
	        this._updateToneFilter();
	    }
	}
};
Sprite_Picture.prototype._createToneFilter = function() {
    this._toneFilter = new ToneFilter();

    var filters = this.filters || [];
    filters.push(this._toneFilter);
    this.filters = filters;
};
Sprite_Picture.prototype._updateToneFilter = function() {
    if (!this._toneFilter) {
        this._createToneFilter();
    }
